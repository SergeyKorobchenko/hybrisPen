ACC.pentlandsupportticketingaddon = {
...
//        $('.js-file-upload__file-name').html($('.js-file-upload__file-name').data("emptyMessage"));
    ...
    onFileChosen: function () {
        $(document).on('change', '#supportTicketForm input[name=files]', function () {
            ...
            if (!ACC.pentlandsupportticketingaddon.isSelectedFilesValid(selectedFile))
            {
	...
	postNewMessage: function () {
		...
		$('.ct-add-new-msg-btn').on('click touchstart', function(e) {
			...
			$.colorbox({
                ...
                onComplete: function () {                	
                    ACC.pentlandsupportticketingaddon.disableMessage($('.js-add-message-status'));
    ...
    bindTicketAddActions: function () {
        $(document).on('click', '#addTicket',
            function (event) {
                ...
                ACC.pentlandsupportticketingaddon.formPostAction("add-support-ticket?ticketAdded=true");
            });
    },
    ...
    bindTicketUpdateActions: function () {
        $(document).on('click', '#updateTicket',
            function (event) {        	
                ...
                ACC.pentlandsupportticketingaddon.formPostAction('?ticketUpdated=true');
            });
    },
    ...
    formPostAction: function (successRedirectUrl) {
        ...
        var selectedFile = document.getElementById('attachmentFiles');
        if (!ACC.pentlandsupportticketingaddon.isSelectedFilesValid(selectedFile)) {
            ACC.pentlandsupportticketingaddon.displayCustomerTicketingAlert({
                type: 'error',
    ...
    processErrorResponse: function (jqXHR) {
            ...
            $.each(jqXHR.responseJSON, function(k,v) {
                var target = '#' + k;
                    ...
                    if (k === 'NotEmpty-supportTicketForm-subject'
                        ...
                        ACC.pentlandsupportticketingaddon.addHasErrorClass();
                    }
                    else {
        ...
        ACC.pentlandsupportticketingaddon.displayCustomerTicketingAlert({type: 'error', messageId: 'supporttickets-tryLater'});
    },
