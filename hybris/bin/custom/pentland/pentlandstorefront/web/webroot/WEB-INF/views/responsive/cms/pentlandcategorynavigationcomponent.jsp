        ...
        <ul class="nav__links nav__links--products js-offcanvas-links">
            <c:forEach items="${categoryNavigation.children}" var="childLevel1">
                <li class="auto nav__links--primary <c:if test="${not empty childLevel1.children}">nav__links--primary-has__sub js-enquire-has-sub</c:if>">
                    <%--<c:forEach items="${childLevel1}" var="childlink1">--%>
                    ...
                    <%--<c:choose>--%>
                        ...
                        <%--<c:when test="${totalSubNavigationColumns == 5}">--%>
                            ...
                            <%--&lt;%&ndash;custom grid class required because 1/5th columns aren't supported by bootstrap&ndash;%&gt;--%>
                                    ...
                                    <c:when test="${true}">
                                        <c:forEach items="${childLevel1.children}" var="childLevel2">
                                            ...
                                            <c:if test="${not empty childLevel2.code}">
                                                <div class="title">${fn:escapeXml(empty childLevel2.name ? childLevel2.code : childLevel2.name)}</div>
                                            </c:if>
