...
<nav class="navigation navigation--bottom js_navigation--bottom js-enquire-offcanvas-navigation" role="navigation">
    ...
    <%--<div class="container">--%>
        <%--<div class="navigation__overflow b-section--full-width">--%>
                                                ...
                                                <c:when test="${true}">
                                                    <c:forEach items="${childLevel1.children}" var="childLevel2">
                                                        ...
                                                        <c:if test="${not empty childLevel2.code}">
                                                            <div class="title">
                                                                <a href="<c:url value="${childLevel2.url}"/>">${fn:escapeXml(empty childLevel2.name ? childLevel2.code : childLevel2.name)}</a>
                                                            </div>
                                                        </c:if>
                                                        <ul class="sub-navigation-list <c:if
                                                            test="${not empty childLevel2.code}">has-title</c:if>">
                                                        <c:forEach items="${childLevel2.children}" var="childLevel3"
                                                                   step="10"
                                                                   varStatus="i">
                                                            <%-- wrap if more than 'component.wrapAfter' rows in one sub column --%>
                                                            <c:if test="${i.index >= wrapAfter}">
                                                                <c:if test="${!i.first}">
                                                                    </ul>
                                                                    </div>
                                                                </c:if>
                                                            ...
                                                            <c:forEach items="${childLevel2.children}" var="childLevel3"
                                                                       begin="${i.index}" end="${i.index + 10 - 1}">
